...
class Rocket:
    def __init__(self, seed, F, consumption, fuel_mass, number_of_engines, rocket_duration, dt, planet_idx, t_launch = 0.0, angle_launch = 0.0):
        ...
        self.planet_direction= np.array([np.cos(planet_angle), np.sin(planet_angle)])
        self.relative_direction = rotate_vector(self.planet_direction, angle_launch)
        
        
        self.rocket_pos = self.planet_pos + self.planet_radius * self.relative_direction
        print(self.planet_pos)
        print(self.rocket_pos, self.rocket_pos - self.planet_pos, "ff")
    
        planet_rotation_speed = 2 * np.pi * self.planet_radius / self.planet_rotation_period

        v_rot_direction = np.array([-self.relative_direction[1], self.relative_direction[0]]) #(-y,x)
...
class RocketSystem:
    def __init__(self, rocket, simulation_time, dt, destination_idx):
        ...
        # self.rocket_vel = np.array([-3.18306807,  5.61995856])
    ...
    def Hohmann(self):
        ...
        Returns:
            ...
            t_1: Time for first boost
            t_2: Time for second boost
        """
        ...
        # v_p = 8.2044554746445 # actual data
        v_circ_1 = np.sqrt(mu / r_p)
        print(v_p, 'v_p')

        # destination parameters
        r2 = np.mean([np.linalg.norm(pos) for pos in self.interpolated_positions[:, self.destination_idx]]) # radius of destination planet (assume circular)
        v2 = np.sqrt(mu / r2)
        ...
        r_vec = self.rocket_initial_pos
        v_vec = self.rocket_initial_vel
        ...
        print(t_since_periapsis)
        
        # time of boosts
        t_1 = T - t_since_periapsis
        t_2 = t_1 + t_12 # 
    ...
    def run(self, boosts = False, calc_energy = False):
        ...
        m = self.rocket.m_rocket / constants.m_sun # assume infinite fuel
        F = self.rocket.F * (constants.yr**2 / constants.AU) / (constants.m_sun) 
        consumption = (self.rocket.consumption / constants.m_sun) * constants.yr  # kg / year
        dv1, dv2, t_1, t_12 = self.Hohmann()
        ...
        for i in range(N-1):
            if boosts:
                ...
                if i == idx_boost1:
                    ...
                    print(f"performing boost 1: boosted (AU/yr): {boost}")
                if i == idx_boost2:
                    print(np.linalg.norm(rocket_vel), 'acutal velocity when boost2')
                    boost = dv2 * v_dir
                    rocket_vel += boost
                    fuel_consumed += consumption * dt_boost2
                    print(f"performing boost 2: boosted (AU/yr): {boost}")
    ...
    def plot_combined(self, boosts):
        ...
        plt.plot([initial_planet1[0], end_planet2[0]], [initial_planet1[1], end_planet2[1]], 'ro', markersize=1) # marks the initial positions of the planets
        ...
        plt.xlabel('x (AU)')
        plt.ylabel('y (AU)')
        plt.title('Rocket Orbit inside Solar System years')
        plt.axis('equal')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        if boosts == True:
            plt.savefig('rocket_and_planets_boosts.png')
        else:
    ...
    def plot_l_distances(self):
        ...
        plt.plot(times, l_destination_list, label=f"Energy over time kJ", alpha=0.6)
        plt.xlabel("Time (years)")
        plt.ylabel(f"L_destination")
        plt.title("L_destination")
        plt.legend(loc="upper right")
        plt.grid(True)
        plt.tight_layout()
...
def main():
    ...
    simulation_time = 8 # Total simulation time in years
    ...
    boosts = True
    rocket_system.run(boosts = True, calc_energy = True)
    pos, vel, orientation = shortcut.get_orientation_data()

    rocket_system.plot_combined(boosts)
    rocket.verify_orientation(rocket_system.rocket_initial_pos, vel, orientation)
    rocket_system.plot_energy(boosts)
    rocket_system.plot_l_distances()
    InterplanetaryTravel = mission.begin_interplanetary_travel()
    InterplanetaryTravel.coast(duration = 6.375465237875894)
    InterplanetaryTravel.boost([ 0.76942339, -0.067075])
    InterplanetaryTravel.coast(duration =7.9740213404184965 - 6.375465237875894)
    InterplanetaryTravel.boost([0.86631105, -0.07533083])
