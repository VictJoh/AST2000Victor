...
def plot_positions(positions, end_landing_site_pos, times, actual = False):
    global t_launch_lander
    t0 = times[0]
    idx_launch_lander = np.argmin(np.abs(t_launch_lander + t0 - times))
    ...
    plt.scatter(end_landing_site_pos[0], end_landing_site_pos[1], label = "Landing Site", color = "purple", s = 20)
    plt.scatter(x_launch, y_launch, label = "Launched Lander", color = "black", s = 20)
    ...
    if actual == True:
        plt.title("Actual Lander Trajectory")
        plt.savefig("Del7/Actual_Lander_trajectory.png")    
    else:
...
def plot_acceleration(accelerations, times):
    ...
    plt.legend()
    plt.savefig("Del7/Acceleration_over_time.png")

def plot_height(heights,times, actual = False):
    heights /= 1000 #[km]
    plt.figure()
    ...
    plt.legend()
    if actual == True:
        plt.title("Actual height over time")
        plt.savefig("Del7/actual_height_over_time.png")
    else:
...
def landing_ast(landing_pos0, picture_time, simulation_time, dt = 1.0, video = True):
    ...
    landing_sequence.adjust_landing_thruster(np.linalg.norm(F_L), 90) 
    ...
    for i in range(N):
        ...
        positions_actual.append(pos.copy())
        heights_actual.append(h.copy())
        landing_sequence.fall(dt*10) 

    if landing_sequence.landing_thruster_activated:
        landing_sequence.verbose = True
        while not landing_sequence.reached_surface and i < 10000:
            t, pos, vel = landing_sequence.orient()
            landing_sequence.fall(dt*10)
            times_actual.append(t)
            positions_actual.append(pos.copy())
            heights_actual.append(h.copy())
    if landing_sequence.reached_surface == False:
        print("did not land")
    else:
        print("landed")

    t_end, pos_end, vel_end = landing_sequence.orient()
    actual_end_landing_site = calc_coords(landing_pos0, picture_time, t_end)
    print(f"deviated by {np.linalg.norm(actual_end_landing_site - pos_end)} m from landing site")
    print(t_end, pos_end, vel_end)
    if video:
        landing_sequence.finish_video()
    return np.array(times_actual), np.array(positions_actual), np.array(actual_end_landing_site), np.array(heights_actual)
...
def main():
    ...
    plot_landing(heights,times)
    times_actual, positions_actual, actual_end_landing_site, heights_actual = landing_ast(landing_site, picture_time, simulation_time, dt = 1.0, video = True)
    plot_positions(positions_actual, actual_end_landing_site, times_actual, actual = True)
    plot_height(heights_actual, times_actual, actual = True)
    plt.show()
